...
var (
	nowFast           = time.Now()
	nowFastUnix       = nowFast.Unix()
	nowNanosecondFast = nowFast.Nanosecond()
	nowFormatFast     = nowFast.Format("2006-01-02 15:04:05")
)
...
func GetNowUnixFast() int64 {
	return nowFastUnix
}

func GetNowNanosecondFast() int {
	return nowNanosecondFast
}

func GetNowFormatFast() string {
	return nowFormatFast
}
...
func DiffTime(a, b time.Time) (year, month, day, hour, min, sec int) {
	...
	if day < 0 {
		// days in month:
		t := time.Date(y1, mon1, 32, 0, 0, 0, 0, time.UTC)
		day += 32 - t.Day()
		month--
	}
	if month < 0 {
